...
{	
	...
	RCC_AHB1PeriphClockCmd(CS_GPIO_CLK,ENABLE);						//开启GPIO模块的时钟
	RCC_AHB1PeriphClockCmd(DC_GPIO_CLK,ENABLE);						//开启GPIO模块的时钟
	RCC_AHB1PeriphClockCmd(RST_GPIO_CLK,ENABLE);					//开启GPIO模块的时钟
	RCC_AHB1PeriphClockCmd(SDA_GPIO_CLK,ENABLE);					//开启GPIO模块的时钟
	RCC_AHB1PeriphClockCmd(SCL_GPIO_CLK,ENABLE);					//开启GPIO模块的时钟
	
	OLED_InitStructure.GPIO_Pin=CS_Pin;										//配置CS端口
	OLED_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
  OLED_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  OLED_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  OLED_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  GPIO_Init(CS_GPIO, &OLED_InitStructure);//初始化GPIO
